...
- для leaf1 - 99 доступна через leaf2
- для leaf2 - 98 доступна через leaf1
Таким образом нужно проверить, будет ли доступен client3 (за leaf3 в 99 влане) с client1 (за leaf1 в 98 влане) и как пойдёт трафик.
```
...
```
ICMP запрос отправляется с leaf1 на leaf2 и следом за ним leaf рассылает type-2 маршруты: mac only и mac+ip с mac vrf и ip vrf рут таргетами.
...
И снова можно было бы подумать, что mac only маршрут не должен сохраниться на leaf1, но нет, они сохраняются, но вот в таблицу mac-адресов или vxlan address-table уже не попадают. Подозреваю, что на нексусах другое поведение, если не включать retain route-target all.
```
...
Добавим хост client3 к leaf1 и устройство, которое будет маршрутизировать между VRF, условный фаервол - R1.
