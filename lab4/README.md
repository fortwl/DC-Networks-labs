...
```
Ничего не происходит, т.к. интервалы согласовываются постоянно, и после изменения на leaf1 противоположная сторона - spine1 эту информацию получает в очередном control packet и теперь ожидает новые пакеты с интервалом в 3 секунды.
